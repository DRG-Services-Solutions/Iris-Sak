...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased text-gray-900 dark:text-gray-100"
        ...
        <div class="h-screen overflow-hidden bg-gradient-to-br from-gray-50 via-slate-50 to-gray-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-800">
            ...
            {{-- BACKDROP oscuro al abrir sidebar --}}
            <div x-show="open" 
            ...
            {{-- SIDEBAR (overlay, nunca empuja contenido) --}}
            @include('layouts.sidebar')

            {{-- CONTENIDO PRINCIPAL (siempre 100% ancho) --}}
            <div class="relative z-10 flex flex-col h-full overflow-y-auto overflow-x-hidden">
                    ...
                    <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
                        <div class="flex items-center gap-3">
                            ...
                            <button @click="toggleSidebar()" 
                                    class="flex-shrink-0 inline-flex items-center justify-center w-10 h-10 rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    title="Menú">
        ...
        <script>
            function sidebarState() {
                return {
                    open: false,
                    init() {
                        // Solo en desktop: restaurar preferencia guardada
                        if (window.innerWidth >= 1024) {
                            this.open = localStorage.getItem('iris_sidebar') === 'true';
                        }
                    },
                    toggleSidebar() {
                        this.open = !this.open;
                        if (window.innerWidth >= 1024) {
                            localStorage.setItem('iris_sidebar', this.open);
                        }
                    },
                    closeSidebar() {
                        this.open = false;
